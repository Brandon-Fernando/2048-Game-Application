...
public class Board {
    ...
    public void addRandomTile() {
        ...
        // int rTile = StdRandom.uniform(0, openSpaces.size());
        // BoardSpot t = openSpaces.get(rTile);
    ...
    public void swipeLeft() {
        ...
        for(int r = 0; r < gameBoard.length; r++){
            for(int c = 1; c < gameBoard[0].length; c++){
                ...
                int x = c-1; 
                while((x >= 0) && (gameBoard[r][x]==0)){
                    gameBoard[r][x+1] = gameBoard[r][x];
                    x--;
                }
                gameBoard[r][x+1] = temp;
            }
        }
    }
    ...
    public void mergeLeft() {
        ...
        for(int r = 0; r < gameBoard.length; r++){
            for(int c = 1; c < gameBoard[0].length;c++){
                int x = c-1;
                int before = gameBoard[r][x];
                int after = gameBoard[r][c];
                if(before == after && before != 0 && after !=0){
                    gameBoard[r][x] = before + after;
                    gameBoard[r][c] = 0;
                }
            }
        }
    ...
    public void transpose() {
        // WRITE YOUR CODE HERE
        int transposedArr[][] = new int[4][4];
        for(int r = 0; r < gameBoard.length; r++){
            for(int c = 0; c < gameBoard[0].length; c++){
                transposedArr[r][c] = gameBoard[c][r];
            }
        }

        for(int r = 0; r < gameBoard.length; r++){
            for(int c = 0; c< gameBoard[0].length;c++){
                gameBoard[r][c] = transposedArr[r][c];
            }
        }
    ...
    public void flipRows() {
        ...
        for(int r = 0; r<gameBoard.length; r++){
            for(int c = 0; c < gameBoard[0].length/2; c++){
                int temp = gameBoard[r][c];
                gameBoard[r][c] = gameBoard[r][gameBoard[0].length - c - 1];
                gameBoard[r][gameBoard[0].length-c-1] = temp;
            }
        }
    }
    ...
    public void makeMove(char letter) {
        // WRITE YOUR CODE HERE
        if(letter == 'L'){ //moves left 
            swipeLeft();
            mergeLeft();
            swipeLeft();
        }
        if(letter == 'R'){ //moves right 
            rotateBoard();
            rotateBoard();
            swipeLeft();
            mergeLeft();
